...
The Abstraction layer maps the normalised technical fields produced by the Ingestion layer to three higher-level concepts that are meaningful for both stakeholder groups: exploitation likelihood, business impact, and urgency. These concepts do not appear in raw scanner output. They are produced by applying deterministic rule tables to combinations of input fields. The use of explicit rule tables is the central architectural expression of DP5: raw scanner data is progressively converted into stakeholder-interpretable abstractions before any explanation is generated. It also directly addresses the objectivity concern raised during the design review. The system does not claim that its ratings are neutral, but it does claim that they are transparent: every rule is visible, every input combination is documented, and every output can be verified by reading the table.

Each rule table is implemented as an explicit conditional function in the prototype. The conditions are evaluated in priority order, from highest risk to lowest. When a required input field is \texttt{None}, the function returns a designated \texttt{Unknown} value rather than defaulting to a low rating or raising an error. This behaviour implements the knowability framework discussed with the academic supervisor: the system distinguishes between a finding that is rated Low because it is genuinely low risk and a finding that cannot be rated because the required evidence is absent. The three rule tables are described in detail below.
...
\label{subsec:business-impact}

Business impact captures the potential organisational consequence of a successful exploitation of this vulnerability. It is derived from the \ac{CVSS} base score and the asset criticality. The \ac{CVSS} score represents the intrinsic severity of the vulnerability in isolation; the asset criticality adjusts this for the importance of the affected system to the organisation. A vulnerability with a high \ac{CVSS} score on a low-criticality development system carries a lower business impact than the same vulnerability on a high-criticality production system. This distinction is important for non-technical stakeholders, who are primarily concerned with business consequence rather than technical severity scores~\cite{cranor2008framework}. The full rule table is shown in Table~\ref{tab:business-impact}.

\begin{table}[h]
\centering
\begin{tabular}{@{}llc@{}}
\toprule
\textbf{CVSS Score} & \textbf{Asset Criticality} & \textbf{Business Impact} \\
\midrule
$\geq 9.0$ & High   & Critical \\
$\geq 9.0$ & Medium & High \\
$\geq 9.0$ & Low    & High \\
$7.0$--$8.9$ & High   & High \\
$7.0$--$8.9$ & Medium & Medium \\
$7.0$--$8.9$ & Low    & Medium \\
$4.0$--$6.9$ & High   & Medium \\
$4.0$--$6.9$ & Medium & Low \\
$4.0$--$6.9$ & Low    & Low \\
...
\bottomrule
\end{tabular}
\caption{Rule table for business impact.}
\label{tab:business-impact}
\end{table}

The \ac{CVSS} score thresholds used in this table are aligned with the severity band definitions published by the National Vulnerability Database: Critical ($\geq 9.0$), High ($7.0$--$8.9$), Medium ($4.0$--$6.9$), and Low ($< 4.0$). Using established conventions rather than arbitrary thresholds makes the rules more defensible and easier to update if the conventions change. The interaction between the score and the asset criticality means that the highest business impact rating, Critical, is only reachable when both the intrinsic severity is at its maximum and the asset is of high organisational importance.
...
\texttt{abstraction.py}      & Implements the three rule tables and returns exploitation likelihood, business impact, and urgency \\
