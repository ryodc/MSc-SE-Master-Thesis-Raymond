...
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
  layer/.style={draw, rounded corners, fill=blue!8, align=center, text width=2.5cm, minimum height=1.5cm, font=\small},
  io/.style={draw, fill=black!10, align=center, text width=2.2cm, minimum height=1.2cm, font=\small}
]
\node[io]    (scan) at (0,0)      {Scanner\\input};
\node[layer] (ing)  at (3,0)      {Ingestion\\[2pt]\footnotesize normalise raw fields};
\node[layer] (abs)  at (6,0)      {Abstraction\\[2pt]\footnotesize map fields to concepts};
\node[layer] (exp)  at (9,0)      {Explanation\\[2pt]\footnotesize produce explanation object};
\node[layer] (sv)   at (12,0)     {Stakeholder view\\[2pt]\footnotesize render per audience};
\node[io]    (tech) at (15.2,1.2) {Technical\\view};
\node[io]    (nt)   at (15.2,-1.2){Non-technical\\view};
\draw[->] (scan) -- (ing);
\draw[->] (ing) -- (abs);
\draw[->] (abs) -- (exp);
\draw[->] (exp) -- (sv);
...
\subsection{Grounding in Stakeholder-Specific Vulnerability Categorization}
\label{subsec:ssvc-grounding}

The three decision trees are not defined ad hoc. They instantiate the deployer decision model of Stakeholder-Specific Vulnerability Categorization (SSVC), the qualitative decision-tree alternative to severity scoring developed by CERT at Carnegie Mellon and adopted by CISA \cite{spring2021ssvc}. SSVC evaluates a small set of decision points and returns an ordered action category, and it requires that every decision point and outcome be explainable to a non-expert, which is the same commitment this artifact makes. Table~\ref{tab:ssvc-mapping} maps the decision points and outcomes used here to their SSVC counterparts.

\begin{table}[h]
\centering
\begin{tabular}{@{}lp{0.34\textwidth}l@{}}
\toprule
\textbf{This artifact} & \textbf{Reads} & \textbf{SSVC decision point} \\
\midrule
Exploitation likelihood & \texttt{exploit\_available}, \texttt{known\_exploited} & Exploitation \\
                        & \texttt{asset\_internet\_facing} & System Exposure \\
Impact                  & \texttt{asset\_environment}, technical severity, \texttt{asset\_business\_critical}, \texttt{asset\_owner\_privileged} & Mission \& Human Impact \\
Urgency (outcome)       & Immediate / Scheduled / Monitor & Immediate / Scheduled / Defer \\
\bottomrule
\end{tabular}
\caption{Mapping of the artifact's decision points and outcomes to the SSVC deployer model.}
\label{tab:ssvc-mapping}
\end{table}

Two deliberate deviations from the standard deployer tree are made, both permitted by SSVC, which explicitly invites stakeholders to define their own decision points and outcomes \cite{spring2021ssvc}. First, the single deployer decision is split into three separate trees for likelihood, impact, and urgency, so that each axis can be explained on its own rather than collapsed into one path. This supports the explanation layer, where the contributing factors are drawn from the individual trees. Second, two organisation-specific decision points, asset business criticality and asset owner privilege, are added from the configuration management database, which the generic SSVC tree does not include.

Because the rules form a finite decision table, the rule base can be checked for the two properties such a table must satisfy: completeness, that every combination of decision-point values maps to a defined outcome, and consistency, that each combination maps to exactly one outcome. Both properties are verified for all three trees by exhaustively enumerating the decision-point value space, which confirms that no input falls through without a rating and that no two rules conflict. The full decision tables are listed in the appendix.
...
\resizebox{\textwidth}{!}{%
\begin{forest}
EL/.style={edge label={node[midway,fill=white,font=\scriptsize,inner sep=1pt]{#1}}},
for tree={draw, rounded corners, font=\small, align=center, edge={->,>=latex},
  l sep=10mm, s sep=5mm},
where n children=0{fill=SkyBlue!18}{},
[\texttt{exploit\_available}?
  [Unknown, EL={None}]
  [\texttt{known\_exploited}?, EL={True}
    [High, EL={True}]
    [\texttt{internet\_facing}?, EL={False}
      [High, EL={True}]
      [Medium, EL={False}]
    ]
  ]
  [\texttt{known\_exploited}?, EL={False}
    [Medium, EL={True}]
    [Low, EL={False}]
  ]
]
...
The technical-severity bands follow the qualitative severity definitions published by the National Vulnerability Database for \ac{CVSS} scores: Critical ($\geq 9.0$), High ($7.0$--$8.9$), Medium ($4.0$--$6.9$), and Low ($< 4.0$); for the rule table, the Critical and High bands are both treated as high technical severity. Reading the asset environment directly, rather than an environment-derived criticality proxy, keeps the assessment honest about its inputs. A complete business-impact rating requires knowledge of the asset itself. Where a finding can be joined to the organisation's configuration management database during ingestion (\cref{sec:ingestion}), two further signals become available: whether the asset is flagged business-critical, and whether its owner is a privileged identity. When either is present the assessment runs in \emph{complete} mode and the Critical level becomes reachable, as shown in Table~\ref{tab:complete-impact}. The two modes share the same environment and severity axes and differ only in whether asset context was available, so a finding without a matching asset record degrades cleanly to the preliminary assessment rather than failing. Data sensitivity remains absent from both sources and is therefore the one component of business impact still identified as future work in Chapter~\ref{ch:conclusion}.
...
EL/.style={edge label={node[midway,fill=white,font=\scriptsize,inner sep=1pt]{#1}}},
for tree={draw, rounded corners, font=\small, align=center, edge={->,>=latex},
  l sep=10mm, s sep=6mm},
where n children=0{fill=SkyBlue!18}{},
[inputs present?
  [Unknown, EL={no}]
  [\texttt{asset\_environment}?, EL={yes}
    [technical severity?, EL={Production}
      [High\\(Critical if elevated), EL={High/Critical}]
      [Medium\\(High if elevated), EL={Medium}]
      [Low\\(Medium if elevated), EL={Low}]
    ]
    [technical severity?, EL={Non-production}
      [Medium\\(High if elevated), EL={High/Critical}]
      [Low\\(Medium if elevated), EL={Medium}]
      [Low\\(Low if elevated), EL={Low}]
    ]
  ]
]
\end{forest}}
\caption{Decision tree for impact. \emph{inputs present?} is true when both the asset environment and a technical severity are available; otherwise the impact is Unknown. A leaf $X$ \emph{(Y if elevated)} gives the preliminary impact $X$, and the complete impact $Y$ when the asset is business-critical or owned by a privileged identity.}
...
Urgency captures how quickly remediation should be prioritised relative to other work. It combines exploitation likelihood with patch availability, asset exposure, and the age of the finding measured against its remediation deadline. The deadline is the per-finding \texttt{threshold\_days} value where the source provides one, and otherwise a configurable default of 90 days; a finding is overdue when \texttt{days\_open} exceeds this threshold. The inclusion of a temporal dimension directly addresses the time element raised during the design review: a finding that has been open well beyond its deadline carries a different operational significance than one detected recently, even if the underlying vulnerability is the same. The full rule table is shown in Table~\ref{tab:urgency}.

\begin{table}[h]
\centering
\begin{tabular}{@{}lllll@{}}
\toprule
\textbf{Likelihood} & \textbf{Patch} & \textbf{Internet-Facing} & \textbf{Overdue} & \textbf{Urgency} \\
\midrule
High    & Any & Any & Any & Immediate \\
Medium  & Yes & Any & Yes & Immediate \\
Medium  & Yes & Any & No  & Scheduled \\
Medium  & No  & Any & Any & Scheduled \\
Low     & Yes & Any & Yes & Scheduled \\
Low     & Yes & Any & No  & Monitor \\
Low     & No  & Any & Any & Monitor \\
Unknown & Any & Yes & Yes & Immediate \\
Unknown & Any & No  & Yes & Scheduled \\
Unknown & Yes & Any & No  & Scheduled \\
Unknown & No  & Any & No  & Monitor \\
\bottomrule
\end{tabular}
\caption{Rule table for urgency. \emph{Overdue} means \texttt{days\_open} exceeds the remediation threshold (the finding's \texttt{threshold\_days}, or a default of 90). A value of \emph{Any} means the input does not affect the outcome on that branch.}
\label{tab:urgency}
\end{table}
...
When exploitation likelihood is Unknown, the tree does not default to the lowest level. It falls back to the two signals that remain trustworthy in the absence of exploit data: asset exposure and the remediation deadline. An internet-facing asset that is past its deadline is rated Immediate, an asset that is past its deadline but not internet-facing is rated Scheduled, a finding still within its deadline for which a patch exists is rated Scheduled, and only a finding with no exposure, no deadline breach, and no patch is rated Monitor. This branch implements the knowability principle in the urgency dimension: a finding whose exploitability cannot be assessed is still prioritised on the evidence that is present, rather than being quietly demoted.
...
where n children=0{fill=SkyBlue!18}{},
[\texttt{exploitation\_likelihood}?
  [Immediate, EL={High}]
  [\texttt{patch\_available}?, EL={Medium}
    [Scheduled, EL={No}]
    [overdue?, EL={Yes}
      [Immediate, EL={Yes}]
      [Scheduled, EL={No}]
  ...
  [internet-facing\\and overdue?, EL={Unknown}
    [Immediate, EL={Yes}]
    [overdue?, EL={No}
      [Scheduled, EL={Yes}]
      [\texttt{patch\_available}?, EL={No}
        [Scheduled, EL={Yes}]
        [Monitor, EL={No}]
      ]
    ]
...
\end{forest}}
\caption{Decision tree for urgency. \emph{overdue?} is true when \texttt{days\_open} exceeds the remediation threshold (\texttt{threshold\_days}, or a default of 90).}
...
\paragraph{Grounding of the template design.} The choice of template-based generation, and the structure of the templates themselves, are grounded in established guidelines rather than authored ad hoc. Template-based realisation is a legitimate strategy rather than an inferior one: van Deemter, Krahmer, and Theune argue that it is not necessarily weaker than fully generative approaches in maintainability, linguistic soundness, or output quality~\cite{vandeemter2005template}, which is the property this system depends on, since it trades fluency for the determinism and auditability that a faithful explanation requires. The set of template families follows the content structure that warning research identifies as effective. Wogalter, Conzola, and Smith-Jackson show that an effective hazard communication should convey the nature of the hazard, its consequences, and the instruction for what to do~\cite{wogalter2002guidelines}. The \texttt{risk\_summary} and \texttt{business\_consequence} fields carry the hazard and its consequence, and the \texttt{action}, \texttt{time\_horizon}, and \texttt{mitigation} fields carry the instruction. The same body of work finds that the perceived severity of the consequence is the strongest determinant of how seriously a hazard is taken, which motivates grading the consequence template by impact level and raising it when the asset context indicates a business-critical or privileged-account system.

The register of the templates, and the decision to render two separate views, follow the principle that a communication succeeds only when it matches the knowledge of the receiver. Cranor's human-in-the-loop framework treats a security message as effective only insofar as the recipient can understand and act on it~\cite{cranor2008humanloop}, and the Communication-Human Information Processing model makes comprehension an explicit stage that depends on matching the vocabulary of the target audience~\cite{wogalter2006chip}. This grounds the non-technical view, which removes identifiers and scores in favour of plain language, against the technical view, which retains precise terminology. That model also prescribes that a message be checked with a representative sample of its intended audience; the wording of the templates was therefore reviewed with non-technical stakeholders, whose feedback retained precise security terms where those were the vocabulary the audience expected. The templates are thus grounded in these guidelines rather than derived from them: the structure follows the warning-content triad, the grading follows the severity finding, the register follows the receiver-centred models, and the exact wording is authored and then validated, not produced by any single source.
...
\centering
\begin{tikzpicture}[
  panel/.style={draw, rounded corners, align=left, text width=6.2cm, inner sep=8pt, font=\small}
]
\node[panel, anchor=north west] (t) at (0,0) {%
  {\bfseries Technical view}\\[4pt]
  CVE identifier\\
  CVSS score (numeric)\\
  Exploitation likelihood (High / Medium / Low)\\
  Business impact\\
  Top factors, each with an evidence pointer\\
  \hspace{1em}(e.g.\ \texttt{exploit\_available: True})\\
  Certainty level\\
  Recommended action\\
  Limitations, if any
};
\node[panel, anchor=north west] (n) at (6.9,0) {%
  {\bfseries Non-technical view}\\[4pt]
  Risk summary sentence\\
  Business consequence sentence\\
  Urgency badge (Immediate / Scheduled / Monitor)\\
  Time horizon sentence\\
  Action, in plain language\\
  Limitations note, if certainty is not Known\\[6pt]
  \itshape No CVE identifiers or CVSS scores shown.
};
\end{tikzpicture}
...
The prototype output for Scenario A is shown in Appendix~\ref{sec:output-screenshots} (Figures~\ref{fig:example-output-tech} and~\ref{fig:example-output-nontech}). The two figures illustrate the core contribution of the Stakeholder View layer: the same underlying finding, structured by the same explanation object, is rendered into two substantially different representations that are each appropriate for their intended audience.
